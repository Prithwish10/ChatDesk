...
it('returns a 422 with an missing firstName, lastName, email and password', async () => {
  ...
  await request(app).post('/api/v1/users/signup').send({ firstName: 'Alpha' }).expect(422);

  await request(app).post('/api/v1/users/signup').send({ lastName: 'test' }).expect(422);

  await request(app).post('/api/v1/users/signup').send({ email: 'alpha1@test.com' }).expect(422);

  await request(app).post('/api/v1/users/signup').send({ password: 'abcdefg' }).expect(422);
});
